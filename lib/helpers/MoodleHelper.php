...
class TurningTechMoodleHelper {
    ...
    public static function authenticateuser($username, $password) {
        global $DB, $CFG;
        if (!empty($CFG->mod_turningtech_debugging)) {
            error_log("------------------TurningTech Error Log----------------------------");
            error_log("Inside Authentication");
        }
        ...
        if ($xx == null) {
            ...
            if ($casvar) {
                // Cas function.
                $cookiedir = tempnam($CFG->tempdir, 'CURLCOOKIE');
            ...
            } else if ($shibvar) { // Check for shiboleth.
                ...
                } else if ($samlvar) { // Check for saml.
            		if (!empty($CFG->mod_turningtech_debugging)) {
                		error_log("----SAML Authentication------");
                	}
                $cookiedir = tempnam($CFG->tempdir, 'CURLCOOKIE');
                $authplug = get_config('moodle', null);
                if (!empty($CFG->mod_turningtech_debugging)) {
                	$url = $authplug->wwwroot . "/" ."auth/saml?wantsurl=".urlencode($authplug->wwwroot);
                } else {
                	$url = $authplug->wwwroot . "/" .'auth/saml/index.php';
                }
                ...
                curl_setopt($curl_connection, CURLOPT_FOLLOWLOCATION, 1);
                // Perform our request.
                $resultold = curl_exec($curl_connection);
                if (!empty($CFG->mod_turningtech_debugging)) {
                		error_log("----SAML Authentication------");
                	}
                $info = curl_getinfo($curl_connection);
                $url = $info['url'];
                if (!empty($CFG->mod_turningtech_debugging)) {
                		error_log("The Auth URL is $url ");
                	}
                // Close the connection.
                curl_close($curl_connection);
                $usernamenew = explode("@", $username);
                if (!empty($CFG->mod_turningtech_debugging)) {
                	$post_data['j_username'] = $usernamenew[0];
                } else {
                	$post_data['j_username'] = $username;
                }
                $post_data['j_password'] = $password;
                ...
                $result2 = curl_exec($curl_connection);
                if (!empty($CFG->mod_turningtech_debugging)) {
                		error_log("Result from data Post ");
                ...
                if ($authsrvrurl.'/idp/profile/SAML2/Redirect/SSO' == $info["url"]) {
                    if ($user = get_complete_user_data('username', $username, $CFG->mnet_localhost_id)) {
                        ...
                        if ($user->id) {
                            ...
                            if (empty($CFG->authpreventaccountcreation)) {
                                // Use manual if auth not set.
                                $user = create_user_record($username, $password, 'saml');
                            }
                        }
                    }
                    foreach ($authsenabled as $hau) {
                    ...
                    if (!empty($CFG->mod_turningtech_debugging)) {
                        ...
                        error_log("------Authentication Log End-----");
                    }   
                    ...
                    if (!empty($CFG->mod_turningtech_debugging)) {
                        error_log("SAML Authentication failed");
                        error_log("------Authentication Log End-----");
                    }
                    return $xx;
                } // End SAML.
                ...
                if (!empty($CFG->mod_turningtech_debugging)) {
                    error_log("None Auth process available");
                    error_log("------Authentication Log End-----");
                }
                return $xx;
            }
            ...
            if (!empty($CFG->mod_turningtech_debugging)) {
                error_log("--- Result from Authentication-----");
                error_log("Authentication successful from Moodle Instance");
                error_log("------Authentication Log End-----");
            }
            return $xx;
        }
